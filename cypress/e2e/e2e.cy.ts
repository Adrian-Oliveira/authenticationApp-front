describe("e2e", () => {
  it("Login", () => {
    cy.intercept("POST", `${Cypress.env("backEndUrl")}/user/login`, {
      statusCode: 200,
      body: {
    ...
    cy.intercept("GET", `${Cypress.env("backEndUrl")}/user/logged`, {
      statusCode: 200,
    });

    cy.intercept("GET", `${Cypress.env("backEndUrl")}/user/profile`, {
      statusCode: 200,
      body: {
        name: "name",
    ...
    cy.get('[data-test-id="login-mail"]')
    ...
    cy.get('[data-test-id="login-totp"]')
      .type("123456")
    ...
    cy.get('[data-test-id="loading-element"]')
      .should("exist")
      .should("be.visible");
  ...
  it("Profile", () => {
    ...
    cy.get('[data-test-id="profile-edit-button"]').should("exist").click();
  ...
  it("Edit page", () => {
    ...
    cy.get('[data-test-id="edit-name"]').clear();
    ...
    cy.get('[data-test-id="edit-bio"]').clear({ force: true });

    cy.get('[data-test-id="edit-bio"]').type("new bio", { force: true });

    cy.get('[data-test-id="edit-bio"]').should("contain.value", "new bio");

    cy.get('[data-test-id="edit-phone"]')
      .should("exist")
      .and("contain.value", "999999999999");

    cy.get('[data-test-id="edit-phone"]').clear();

    cy.get('[data-test-id="edit-phone"]').type("888888888888", { force: true });

    cy.get('[data-test-id="edit-phone"]').should(
      "contain.value",
      "888888888888",
    );

    cy.get('[data-test-id="edit-photo"]').selectFile(
      "./cypress/fixtures/profileImage.jpg",
      { force: true },
    );
    ...
    cy.intercept("PUT", `${Cypress.env("backEndUrl")}/user/profile`, {
      statusCode: 200,
      body: {
        name: "new name",
        email: "email@email.com",
  ...
  it("2FA Page to disable 2fa", () => {
    ...
    cy.intercept("GET", `${Cypress.env("backEndUrl")}/user/twofactor`, {
      statusCode: 200,
      body: {
        alreadyHave2FA: true,
      },
    ...
    cy.get('[data-test-id="2fa-input"]').should("exist");

    cy.get('[data-test-id="2fa-disableButton"]').should("exist");
  });
  ...
  it.only("2FA Page to enable 2fa", () => {
    ...
    cy.get('[data-test-id="2fa-qrcode"]').should("exist");

    cy.get('[data-test-id="2fa-input"]').should("exist");

    cy.get('[data-test-id="2fa-enableButton"]').should("exist");
  });
});
